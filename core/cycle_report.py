...
def build(cycle_start: float | None = None, cycle_id: str | None = None,
          brain_writer=None) -> dict:
    ...
    try:
        if brain_writer is None:
            raise LookupError("no brain in the spine")
        digest = "\n".join(
...
def to_markdown(rep: dict) -> str:
    ...
    # ── ЗАСТОЙ = НЕНАПРЕДЪК (Емил, 11 септ. 2026, #59) ─────────────────────
    # Ос, която стои далеч от целта и не мърда, не е „нищо ново" — тя е
    # ненапредък. Тук се изброява всяка нощ, за да се търси решение.
    try:
        from core.alarm_bands import stagnant_axes as _stag
        _st = _stag()
        if _st:
            out += ["## Застой — ненапредък", "",
                    f"- {len(_st)} ос(и) стоят далеч от целта си и не се движат; търси се решение:"]
            for r in _st[:12]:
                out.append(f"- **{r['axis']}** — {r.get('why')}")
            out.append("")
    except Exception:
        pass
