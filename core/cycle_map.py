...
STEPS = [
    ...
    ("daily_tier", "2.52", "Същото четене от 2.5, запазено ПО ДНИ вместо презаписано: по "
     "един ред на (индикатор, дата). Без мрежа — числата вече са на диска. Пренесена "
     "стойност НЕ е наблюдение и не влиза.",
    ...
    # ДОБАВЕНА 13 сеп 2026. core/output_contracts.py съществуваше и беше зелен
    # с НИКОЙ, който да го вика — одитът го изброяваше сред непланираните
    # производители, което е учтивият начин да се каже, че проверка, която не се
    # пуска, не съществува. Вписана И ТУК, а не само в config/cycle_phases.json:
    # предната стъпка, добавена само там, се записа като unmapped_checkpoint.
    ("output_contracts", "20.08", "Съди изходите на другите стъпки; не поправя нищо и не спира нощта.",
     ["memory/output_contracts_latest.json"], False),
    ...
    ("needs_auth", "25.37", "Източници, които чакат ключ — веднъж седмично, с линка "
     ...
     [], False),
    # ДОБАВЕНА 11 септ. 2026 (задача #60). Ученето ВЪВ нощта: дневният tier, оценка и
    # пренастройка на learner-а, запечатване на утрешните прогнози, лигата на облачните
    # умове. Преди нея огледалото, дебрифът и отчетът, за да видят наученото тази нощ.
    # ДВА АРТЕФАКТА, НЕ ЕДИН (11 септ. 2026). Редът обявяваше само
    # learn_world_latest.json; _league() пише и memory/backend_order_measured.json
    # (scripts/backend_league.ORDER, чрез core/learn_world.py). Колоната
    # `produces` се пълни само където е ПРОВЕРЕНА (ред 21) — а непълна е също
    # толкова неточна, колкото измислена: kept_promise() съди стъпката по това,
    # което е обявила, и не би забелязала, ако редът на умовете спре да се пише.
    # И ПАК ЕДИН, 20 септ. 2026 — другият. learn_world_latest.json беше разписка:
    # никой не я четеше, стъпката спря да я пише, редът я маха оттук. Остава
    # backend_order_measured.json, който се пише при всяко минаване на _league()
    # и се чете от core/groq_backend.py, така че kept_promise() пак има по какво
    # да съди стъпката — и ако лигата падне, вече го казва вместо да го скрие
    # зад разписка, която се пишеше и когато нищо не е научено.
    ("learn_world", "25.43", "Ученето във нощта: tier -> оценка -> пренастройка на alpha -> "
     "утрешни прогнози -> ред на облачните умове. Сутрешната задача остава като догонване.",
     ["memory/backend_order_measured.json"], False),
