...
STEPS = [
    ...
    ("brain_briefing", "-0.9", "Мозъкът пише плана на деня: фокус, подозрение, свой тест за успех.",
     ["memory/brain_cycle_plan.json"], True),
    ("telegram_approvals", "-0.7", "Прилага човешките отговори OK/NO преди цикъла.",
     ["memory/approvals_ledger.jsonl", "memory/telegram_offset.json"], False),
    ("notify_patches_and_initiatives", "-0.5", "Известява какво чака одобрение.",
     ["memory/pending_approvals.json"], False),
    ("body_scan", "0", "CPU/RAM/VRAM/диск/модели -> адаптивни директиви.",
     ["memory/body_scan_latest.json", "memory/adaptive_directives.json"], True),
    ("canon_load", "0.3", "Зарежда канона и го сверява с хардкоднат SHA-256.",
    ...
    ("grounding_ledger", "2.7", "Записва разминаването LLM срещу данни.",
