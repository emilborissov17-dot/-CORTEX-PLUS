...
def read_pantry(day: str | None = None, base=None) -> dict:
    """What the pantry holds for one day. READS. Never fetches.

    WHY THE CYCLE STOPPED FETCHING (13 Sep 2026). run() below is the whole
    internet: RSS, GitHub, arXiv, the YouTube API, yt-dlp, Whisper and Playwright.
    Measured on the night of 12 Sep it took 1441.7s and spawned 32 child processes
    inside the cycle, and made 311 outbound connections. The same machine died of
    memory three times on 13 Sep — twice while a catch-up was spawned into it, once
    at 138 MB free. A browser with thirty processes in the middle of the night, on
    a machine that cannot hold its own cycle, is not intelligence gathering; it is
    the reason the night ends.

    So the fetching moves OUT of the cycle and this reads what the day left:

        openclaw_queue/cards/<day>_cards.jsonl   judged observations
        news/<day>/*.json                        per-axis news the fetcher wrote
        memory/transcript_cache/*.json           transcripts, cross-cycle

    AN EMPTY PANTRY IS AN ANSWER, not an error. It says so and the cycle walks on.
    The forbidden fallback is fetching "just this once because it is empty" — that
    is the behaviour being removed, and it would come back on exactly the nights
    the machine is least able to afford it.

    run() is untouched and still does everything it did. It is simply no longer
    called from inside the night.
    """
    import datetime as _dt
    root = Path(base) if base else BASE_DIR
    day = day or _dt.date.today().isoformat()

    cards_file = root / "openclaw_queue" / "cards" / f"{day}_cards.jsonl"
    cards = []
    ...
    news_dir = root / "news" / day
    news_files = sorted(news_dir.glob("*.json")) if news_dir.is_dir() else []

    cache_dir = root / "memory" / "transcript_cache"
    transcripts = sorted(cache_dir.glob("*.json")) if cache_dir.is_dir() else []
    fresh = []
    for f in transcripts:
        try:
            if _dt.datetime.fromtimestamp(f.stat().st_mtime).date().isoformat() == day:
                fresh.append(f)
        except OSError:
            pass
    ...
    rec = {"day": day,
           "cards": len(cards),
           "card_axes": sorted({c.get("axis") for c in cards if c.get("axis")}),
           "news_files": len(news_files),
           "news_axes": sorted(f.stem.replace("_news_latest", "") for f in news_files),
           "transcripts_total": len(transcripts),
           "transcripts_today": len(fresh),
           "fetched": False, "children_spawned": 0, "browser": False}
    rec["empty"] = not (cards or news_files or fresh)

    if rec["empty"]:
        print(f"[INTERNET] килерът е празен за {day} — нищо не е сложено в него днес; "
              f"цикълът продължава")
    else:
        print(f"[INTERNET] pantry {day}: {rec['cards']} card(s), "
              f"{rec['news_files']} news file(s), {rec['transcripts_today']} "
              f"transcript(s) today of {rec['transcripts_total']} cached")
        if rec["news_axes"]:
            print(f"[INTERNET]   axes with news: {', '.join(rec['news_axes'][:8])}"
                  + (" …" if len(rec["news_axes"]) > 8 else ""))
    return rec
