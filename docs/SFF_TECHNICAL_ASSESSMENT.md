...
| "Action in the world" | **CORRECTED 2026-08-17 — the original entry was false.** It read: *"Not implemented. System observes and generates local files. No external API writes, no messages sent, no integration with any system outside this machine."* All three of those clauses are wrong. **Messages are sent:** `experiments/needs/needs_report.py` and `experiments/needs/approve_reader.py` POST to `api.telegram.org/…/sendMessage` — the nightly brief and the approval channel both go to a phone. **External API writes exist:** `github_publisher.py` issues `requests.put` against `api.github.com`, which is a write to a public repository. **And there is an outward bridge skeleton:** `agents/openclaw_bridge.py` (commit `565cefa`, 2026-07-18) holds a fail-closed policy gate and an audit-first `submit_action()`. What remains true is narrower and should be stated as such: the OpenClaw bridge **cannot act** — `_execute()` raises `NotImplementedError` and has no caller outside its tests — and every outward action that does happen is either a message to the operator or a publish, both of which a human can see. |
