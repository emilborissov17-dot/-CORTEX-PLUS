...
# Уверяваме се, че CORTEX++ коренът е в sys.path,
# за да работят импортите (cortex, core, cortex_llm и т.н.)
...
def run_batch_phase():
    """
    Batch фаза на Cortex4_v2: автоматични резюмета на Fresco/other sources.
    Може да бъде включвана/изключвана с ENABLE_BATCH_FRESCO_SUMMARIES.
    """
...
def llm_call(prompt: str) -> str:
    """
    Абстракция на LLM повикване.
    В бъдеще тук може да закачим:
    - локален модел,
    - OpenClaw runtime,
    - MeTTa/PeTTa/Prolog слой,
    без да променяме agent_loop.
    """
    return cortex_llm.run_llm(prompt)
