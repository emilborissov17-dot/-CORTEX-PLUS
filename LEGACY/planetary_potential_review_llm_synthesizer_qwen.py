...
def load_planetary_snapshots() -> Dict[str, Any]:
    """
    Чете всички *snapshot_latest.json под planetary/* и ги връща като dict:
    { "ENERGY": {...}, "WATER": {...}, ... }
    """
    snapshots: Dict[str, Any] = {}
    if not PLANETARY_DIR.exists():
        return snapshots

    for subdir in PLANETARY_DIR.iterdir():
...
ЗАДАЧА:
  ...
  - Използваш реалните planetary snapshot-и и контекста по-долу за да попълниш LEVEL и SIGNALS.
...
def main() -> None:
    ...
    # режем Thinking... и всичко преди истинския Markdown
    cleaned_body = strip_to_planetary_markdown(raw)

    # проста структурна проверка
