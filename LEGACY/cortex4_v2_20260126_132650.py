...
CONFIG_DIR = os.path.join(BASE_DIR, "config")  # за лога и други конфигурации
SELF_IMPROVEMENT_FILE = os.path.join(CONFIG_DIR, "self_improvement_suggestions.txt")
PATCH_DIR = CONFIG_DIR  # patch_*.txt в config/
...
def append_log(entry: str):
    """Пише лог за тази сесия във фиксиран файл cortex4_v2_current.log."""
    ensure_dirs()
    try:
...
def process_raw_files_and_summarize():
    """
    Чете суровите глави и прави авто-резюмета в 02_summary_bg, с логване и метрики:
    - размер на входните и изходните файлове;
    - време за изпълнение на партидната част.
    """
    ...
    for fname in files:
        ...
        try:
            ...
            append_log(f"[METRIC] SUMMARY_FILE_SIZE bytes {summary_size} | {out_path}")
...
def make_patch_proposal_path():
    """Генерира име на patch файл в CONFIG_DIR."""
    ensure_dirs()
...
def build_patch_proposal(ts: str, source_log: str, user_inputs: int, errors: int,
    ...
    """Генерира текст за patch предложение на база метриките от лога."""
    reasons = []
    suggestions = []
...
def append_self_improvement_entry(source_log: str):
    """Self-improvement блок + patch, плюс GOAL_ALIGNMENT от целевите файлове в BASE_DIR."""
    ensure_dirs()
    ...
    # GOAL_ALIGNMENT – четем файловете от BASE_DIR
    ...
    if summaries_ok > 0:
        goal_alignment_lines.append(
            f"GOAL_NOTE: В тази сесия се работи по {summaries_ok} глави. Това разширява знанието, но остава в рамките на наличните сурови файлове."
        )
    else:
        goal_alignment_lines.append(
...
def handle_reflect_last_log():
    """REFLECT работи винаги върху cortex4_v2_current.log."""
...
def interactive_loop():
    """Прост интерактивен цикъл."""
    print("\nВлизам в интерактивен режим (CORTEX++ балон).")
    print("Напиши команда/въпрос или 'exit' за изход.")
    print("Специална команда: 'REFLECT LAST_LOG' за self-improvement + patch.\n")
