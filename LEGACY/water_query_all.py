...
ACTIVE_PATH = BASE_DIR / "water_sources_active.json"
OUTPUT_DIR = BASE_DIR / "knowledge" / "water_snapshots"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


def load_active_sources():
    if not ACTIVE_PATH.exists():
        print(f"WATER_QUERY: няма файл {ACTIVE_PATH}")
        return []
    ...
    with ACTIVE_PATH.open("r", encoding="utf-8") as f:
        try:
            ...
            print(f"WATER_QUERY: грешка при парсване на active JSON: {e}")
            return []

    if not isinstance(data, list):
        print("WATER_QUERY: очаквах списък от активни източници.")
        return []
...
def main():
    print("WATER_QUERY: старт на заявка към всички активни източници...")

    active_sources = load_active_sources()
    print(f"WATER_QUERY: активни източници: {len(active_sources)}")
    ...
    summary_path = OUTPUT_DIR / "water_query_summary.json"
    ...
    print(f"WATER_QUERY: обобщение записано в {summary_path}")
