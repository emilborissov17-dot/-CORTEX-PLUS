...
def publish_verified_hypotheses() -> int:
    ...
    md  = f"# Verified Hypotheses — {date}\n\n"
    md += "> Auto-generated by CORTEX++ citation_verifier\n\n"
    md += "| Axis | Prediction | Value | Date | Status | Confidence |\n"
    md += "|------|-----------|-------|------|--------|------------|\n"
    for h in assessed:
        text = h.get("hypothesis_text", "")[:120].replace("|", "╎")
        md += (f"| {h.get('axis', '?')} | {text} | {h.get('predicted_value', '?')} "
               f"| {h.get('prediction_date', '?')} | {h.get('verification_status', '?')} "
               f"| {h.get('confidence', '?')} |\n")
    md += "\n## Detail\n\n"
    for h in assessed:
        ...
        md += f"- **Prediction date:** {h.get('prediction_date', '?')}`\n"
