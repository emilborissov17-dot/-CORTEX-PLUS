...
# ROOT = директната папка CORTEX++, където е cortex4_v2.py
...
from cortex import memory
from core import cortex_llm
from core import goals  # за достъп до дървото и контекста на целите

# === БАЗОВИ ПЪТИЩА ===
BASE_DIR = str(ROOT)

# === GOAL FILES / CIVILIZATION CONTEXT ===
GOAL_FILES = {
    "civilization_goal": "civilization_goal.txt",
    "civilization_vision": "civilization_vision.txt",
    "goal_summary_full": "goal_summary.txt",
    "goal_summary_short": "goal_summary_short.txt",
    "core_role": "core_role.txt",
}

def load_goal_context() -> str:
    parts = []
    parts.append("=== CIVILIZATION GOALS & ROLE CONTEXT (CORTEX++) ===")
    for label, fname in GOAL_FILES.items():
        path = ROOT / fname
        if not path.exists():
            parts.append(f"\n[{label}] MISSING FILE: {fname}\n")
            continue
        try:
            txt = path.read_text(encoding="utf-8").strip()
        except Exception as e:
            parts.append(f"\n[{label}] ERROR READING {fname}: {e}\n")
            continue
        parts.append(f"\n=== {label.upper()} ({fname}) ===\n{txt}\n")
    return "\n".join(parts)

# === ФЛАГОВЕ ЗА РЕЖИМИ ===
ENABLE_BATCH_FRESCO_SUMMARIES = True  # ако искаш да пропускаш batch фазата, сложи False
...
# INTERNET SNAPSHOTS ЗА AXIS / REVIEW
INTERNET_SNAPSHOTS_BASE = KNOWLEDGE_DIR / "internet_snapshots"
ENERGY_REVIEW_INTERNET_DIR = INTERNET_SNAPSHOTS_BASE / "ENERGY_REVIEW"
FOOD_REVIEW_INTERNET_DIR = INTERNET_SNAPSHOTS_BASE / "FOOD_REVIEW"
HUMAN_WELL_BEING_REVIEW_INTERNET_DIR = INTERNET_SNAPSHOTS_BASE / "HUMAN_WELL_BEING_REVIEW"
WATER_REVIEW_INTERNET_DIR = INTERNET_SNAPSHOTS_BASE / "WATER_REVIEW"

# Общ журнал за ресурсни ревюта (планета)
RESOURCE_JOURNAL_FILE = ROOT / "journal_resource_act.txt"

# === LLM GATEWAY (локален cortex_llm_resource.py) ===
LLM_RESOURCE_SCRIPT = ROOT / "cortex_llm_resource.py"

# === AXIS REVIEW / SESSION LOGS ===
SESSION_LOGS_DIR = ROOT / "logs"
AXIS_RUNNER_SCRIPT = ROOT / "run_session_status.py"
...
def ensure_dirs():
    ...
    for d in [
        ...
        ENERGY_REVIEW_INTERNET_DIR,
        FOOD_REVIEW_INTERNET_DIR,
        HUMAN_WELL_BEING_REVIEW_INTERNET_DIR,
        WATER_REVIEW_INTERNET_DIR,
    ]:
...
def load_control_rules():
    ...
    rules_path = ROOT / "cortex_control.rules"
...
def run_batch_phase():
    append_log("=== СТАРТ НА CORTEX4_V2 СЕСИЯ (BATCH) ===")
    ...
    append_log("=== КРАЙ НА ОБРАБОТКАТА НА ФАЙЛОВЕТЕ (BATCH) ===")
...
def load_json_file(path: Path):
    if not path.exists():
        print(f"[JSON] Файлът не съществува: {path}")
        append_log(f"[JSON] missing: {path}")
        return None
    try:
        with path.open("r", encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        print(f"[JSON ERROR] Проблем при четене на {path}: {e}")
        append_log(f"[JSON ERROR] {path} -> {e}")
        return None
...
def load_internet_snapshots_for_axis(axis: str) -> str:
    axis = axis.upper()
    if axis == "ENERGY":
        folder = ENERGY_REVIEW_INTERNET_DIR
    elif axis == "FOOD":
        folder = FOOD_REVIEW_INTERNET_DIR
    elif axis in ("HUMAN_WELL_BEING", "HUMAN_WELL_BEING_REVIEW"):
        folder = HUMAN_WELL_BEING_REVIEW_INTERNET_DIR
    elif axis == "WATER":
        folder = WATER_REVIEW_INTERNET_DIR
    else:
        return ""
    ...
    for p in sorted(folder.iterdir()):
        if p.is_file() and p.name.lower().endswith("_snapshot.txt"):
            try:
...
def build_resource_prompt(domain_name_bg: str,
    ...
    if axis_name:
        internet_layer = load_internet_snapshots_for_axis(axis_name)
        if internet_layer:
            lines.append(f"ДОПЪЛНИТЕЛНИ INTERNET_SNAPSHOTS ЗА ОС {axis_name.upper()}:")
            lines.append(internet_layer)
            lines.append("")
...
def call_internal_llm_resource(domain_id: str, prompt: str) -> str:
    ...
    tmp_in = ROOT / f"tmp_resource_prompt_{domain_id}_{ts}.txt"
    tmp_out = ROOT / f"tmp_resource_response_{domain_id}_{ts}.txt"
    ...
    cmd = f'python "{LLM_RESOURCE_SCRIPT}" --domain={domain_id} --input="{tmp_in}" --output="{tmp_out}"'
    try:
        os.system(cmd)
    except Exception as e:
...
def run_planet_review(domain_id: str,
    ...
    if summary_obj is None:
        print(
            f"CORTEX++ / {domain_name_bg.upper()} REVIEW: "
            f"няма валидно {summary_path.name} (fallback, продължавам само със snapshots)."
        )
...
def load_fresco_config():
    global CURRENT_MODE
    config_path = ROOT / "config_fresco_agent.json"
...
# ======================================================================
# AXIS STATUS / SESSION LOGS – (декларации за да не се чупи interactive_loop)
# ======================================================================

def run_axis_status_session():
    if not AXIS_RUNNER_SCRIPT.exists():
        print("[AXIS] Няма run_session_status.py.")
        return None
    ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    log_name = f"SESSION_{ts}_LOG.txt"
    log_path = SESSION_LOGS_DIR / log_name
    SESSION_LOGS_DIR.mkdir(parents=True, exist_ok=True)
    cmd = f'python "{AXIS_RUNNER_SCRIPT}" > "{log_path}"'
    os.system(cmd)
    return str(log_path)
...
def get_latest_session_log():
    if not SESSION_LOGS_DIR.exists():
        print("[AXIS] Няма SESSION логове.")
        return None
    logs = [p for p in SESSION_LOGS_DIR.iterdir() if p.is_file() and p.name.startswith("SESSION_")]
    if not logs:
        print("[AXIS] Няма SESSION логове.")
        return None
...
def print_axis_status_summary(log_path: str):
    print(f"[AXIS] SESSION LOG: {log_path}")


def show_axis_plan_from_session(log_path: str):
    print(f"[AXIS PLAN] SESSION LOG: {log_path}")


# ======================================================================
# MeTTaClaw-СТИЛ LLM CALL
# ======================================================================

def llm_call(prompt: str) -> str:
    goal_ctx = load_goal_context()
    full_prompt = f"""{goal_ctx}

=== HUMAN INPUT (CORTEX++ INTERACTIVE) ===
{prompt}
"""
    return cortex_llm.run_llm(full_prompt)
...
def interactive_loop():
    ...
    print(" - 'SHOW TREE'                  – показва дървото ЧОВЕК–ПЛАНЕТА–ЦИВИЛИЗАЦИЯ–КОСМОС.")
    print("\nPLANET DOMAINS:")
    ...
    while True:
        ...
        if cmd == "SHOW TREE":
            print(goals.format_civilization_tree())
            continue
        ...
        # СВОБОДЕН ВЪПРОС -> MeTTaClaw слой с памет (agent loop)
        try:
            reply = agent_iteration(user_input, llm_call)
        except Exception as e:
            reply = f"[AGENT ERROR] Грешка при agent_iteration: {e}"
        print(reply)
        append_log(f"[AGENT RESPONSE] {reply}".replace(os.linesep, " "))
...
def main():
    ...
    if ENABLE_BATCH_FRESCO_SUMMARIES:
        ...
        print("\n[INFO] Batch фазата (Fresco auto-summaries) е изключена за тази сесия.")
        append_log("[INFO] Пропусната batch фаза (Fresco summaries).")
