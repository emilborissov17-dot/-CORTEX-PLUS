...
class Memory:
    def __init__(self, path="cortex1_memory.json"):
        self.path = path
        self.episodes = []
        self.load()
...
class Interpreter:
    TOPIC_KEYWORDS = {
        ...
        "смисъл": ["смисъл", "защо", "цел", "безсмислено", "безнадеждност"],
...
# ---------- Cortex-1 Agent (с вътрешен self-talk) ----------
...
def main():
    print("Cortex-1: когнитивен агент с вътрешен self-talk (експериментална версия).")
    ...
    while True:
        ...
        if user_text.lower() in ["изход", "exit", "quit"]:
            print("Cortex-1: Спирам за сега. Ще помня последните епизоди локално.")
            break
        ...
        print(reflection)
        print("\n[Вътрешни мисли на агента (self-talk):]")
