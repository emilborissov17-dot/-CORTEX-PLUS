...
# Пътове като Path за новите функции
BASE_PATH = Path(BASE_DIR)
ENERGY_SNAPSHOTS_DIR = BASE_PATH / "knowledge" / "energy_snapshots"
...
# === НОВИ ПОМОЩНИ ФУНКЦИИ ЗА ENERGY REVIEW ===
...
def load_energy_snapshots_summary():
    summary_path = ENERGY_SNAPSHOTS_DIR / "energy_query_summary.json"
...
def load_energy_snapshot_file(filename: str):
    snap_path = ENERGY_SNAPSHOTS_DIR / filename
...
def dummy_llm_call(prompt: str) -> str:
    """
    Временна функция за LLM повикване.
    ТУК ТРЯБВА ДА СВЪРЖЕШ КЪМ ЛОКАЛНИЯ СИ LLM (GPT4All/Ollama/Qwen).
    Засега връща placeholder.
    """
    # TODO: замени това с реално извикване на cortex_llm.py / локален модел
    return (
        "ENERGY_STATE_SUMMARY: [тук трябва да дойде реален LLM отговор]\n"
...
def cmd_energy_review():
    """
    ENERGY REVIEW:
    - Чете последните енергийни snapshots.
    - Чете визия/политика/поведение.
    - Прави анализ + self-review + предложения за подобрение.
    Записва резултата в journal_resource_act.txt и self_improvement_suggestions.txt.
    """

    print("CORTEX++ / ENERGY REVIEW: стартирам анализ на енергийния домейн...")

    vision_text = read_text_file(BASE_PATH / "civilization_vision.txt")
    policy_text = read_text_file(BASE_PATH / "resource_policy.txt")
    behavior_text = read_text_file(CONFIG_PATH / "agent_behavior_rules.txt")
    axes_spec_text = read_text_file(CONFIG_PATH / "agi_axes_spec.txt")
    axes_status_text = read_text_file(CONFIG_PATH / "agi_axes_status.txt")

    summary = load_energy_snapshots_summary()
    if not summary or "sources" not in summary:
        print("CORTEX++ / ENERGY REVIEW: няма валидно energy_query_summary.json.")
        return

    sources = summary.get("sources", [])
    if not sources:
        print("CORTEX++ / ENERGY REVIEW: в summary няма източници.")
        return
    ...
    for src in selected_sources:
        fname = src.get("file")
        snap = load_energy_snapshot_file(fname) if fname else None
        if snap:
            ...
            snapshots_data.append({
                ...
                "text_sample": data.get("text_sample", "")[:4000],
            })
    ...
    prompt_parts.append("ТИ СИ: CORTEX++ – агент за устойчиво бъдеще и достоен живот за всички.")
    prompt_parts.append("ЗАДАЧА: Направи етичен и системен анализ на енергийната система на света на база данните по-долу и визията/правилата.")

    prompt_parts.append("\n=== ВИЗИЯ ЗА ЦИВИЛИЗАЦИЯТА ===\n")
    prompt_parts.append(vision_text[:4000])

    prompt_parts.append("\n=== ПОЛИТИКА ЗА РЕСУРСИТЕ ===\n")
    prompt_parts.append(policy_text[:4000])

    prompt_parts.append("\n=== ПОВЕДЕНЧЕСКИ ПРАВИЛА НА АГЕНТА ===\n")
    prompt_parts.append(behavior_text[:3000])

    prompt_parts.append("\n=== ОСИ НА AGI И СЪСТОЯНИЕ ===\n")
    prompt_parts.append(axes_spec_text[:3000])
    prompt_parts.append("\n--- ТЕКУЩО СЪСТОЯНИЕ НА ОСИТЕ ---\n")
    prompt_parts.append(axes_status_text[:2000])

    prompt_parts.append("\n=== ЕНЕРГИЙНИ ДАННИ (SNAPSHOTS) ===\n")
    for snap in snapshots_data:
        prompt_parts.append(f"\n--- ИЗТОЧНИК: {snap['source_name']} ({snap['source_url']}) ---")
        prompt_parts.append(f"STATUS / CONTENT: {snap['status_code']} / {snap['content_type']}")
        if snap.get("notes"):
            prompt_parts.append(f"NOTES: {snap['notes']}")
        prompt_parts.append("\nTEXT SAMPLE:\n")
        prompt_parts.append(snap["text_sample"])

    prompt_parts.append(
        "\n=== ИНСТРУКЦИЯ КЪМ ТЕБ ===\n"
        "1) Направи кратко, но съдържателно РЕЗЮМЕ на състоянието на енергийния домейн, както го виждаш от тези данни.\n"
        "2) Оцени ДО КОЛКО това състояние е в съзвучие или в конфликт с визията за достоен, устойчив живот за всички.\n"
        "3) Открои 3–7 КЛЮЧОВИ ПРОБЛЕМА/РИСКА и 3–7 ВЪЗМОЖНОСТИ/ЛОКОМOТИВИ ЗА ПОДОБРЕНИЕ.\n"
        "4) Направи SELF-REVIEW: къде твоите текущи правила, оси и начин на мислене са недостатъчни, за да решаваш този домейн добре.\n"
        "5) Формулирай КОНКРЕТНИ ПРЕДЛОЖЕНИЯ за самоусъвършенстване – какво да се промени в:\n"
        "   - agent_behavior_rules.txt\n"
        "   - agi_axes_spec.txt / agi_axes_status.txt\n"
        "   - resource_policy.txt (ако е необходимо)\n"
        "6) Структурирай отговора в блокове:\n"
        "   A) ENERGY_STATE_SUMMARY\n"
        "   B) ALIGNMENT_WITH_VISION\n"
        "   C) RISKS_AND_OPPORTUNITIES\n"
        "   D) SELF_REVIEW\n"
        "   E) SELF_IMPROVEMENT_SUGGESTIONS\n"
    )

    full_prompt = "\n".join(prompt_parts)

    print("CORTEX++ / ENERGY REVIEW: изпращам заявка към вътрешния LLM...")
    # TODO: свържи dummy_llm_call към реален LLM
    llm_response = dummy_llm_call(full_prompt)

    ts = datetime.utcnow().isoformat()
    header = f"\n\n=== ENERGY REVIEW @ {ts} UTC ===\n"

    journal_path = BASE_PATH / "journal_resource_act.txt"
    with journal_path.open("a", encoding="utf-8") as f:
        f.write(header)
        f.write(llm_response)
        f.write("\n")

    self_imp_path = CONFIG_PATH / "self_improvement_suggestions.txt"
    with self_imp_path.open("a", encoding="utf-8") as f:
        f.write(header)
        f.write("SOURCE: ENERGY REVIEW\n")
        f.write(llm_response)
        f.write("\n")

    print(f"CORTEX++ / ENERGY REVIEW: записано в {journal_path.name} и {self_imp_path.name}.")
...
def interactive_loop():
    ...
    while True:
        ...
        response = (
            ...
            "Специално: можеш да използваш командите 'REFLECT LAST_LOG', 'MODE FRESCO', 'SHOW MODE' и 'ENERGY REVIEW'."
        )
