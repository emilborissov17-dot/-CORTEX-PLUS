...
def load_internet_snapshots_for_axis(axis: str) -> str:
    """
    Чете всички *_snapshot.txt файлове от knowledge/internet_snapshots/<AXIS>_REVIEW/
    и ги връща слепени като текст. Ако няма – връща празен стринг.
    """
    axis = axis.upper()
    if axis == "ENERGY":
...
def call_internal_llm_resource(domain_id: str, prompt: str) -> str:
    ...
    # ВАЖНО: на Windows ползваме 'python', не 'python3'
...
def cmd_materials_review():
    ...
    run_planet_review(
        ...
        extra_context=extra,
        axis_name=None,  # засега нямаме internet_snapshots за този домейн
    )
