...
def main():
    ...
    # ── 25.43. УЧЕНЕТО ВЪВ НОЩТА (11 септ. 2026, задача #60) ──────────────
    # Таблицата „цикъл × 14 точки": от 67 нощни стъпки шест служеха на ученето, а
    # истинското учене живееше в сутрешна задача от 9 стъпки, шест часа след данните.
    # Тук, в G_LEARN, след нощното теглене и преди огледалото, дебрифа и отчета —
    # за да видят и трите какво е научено тази нощ. Всяка част е fail-open.
    beat("learn_world", "25.43")
    def _learn_world():
        from core.learn_world import run as _lw
        _r = _lw()
        print(f"[FAST_CYCLE] learn_world -> {_r.get('ok_parts')} part(s) OK"
              + (f", FAILED: {', '.join(_r['failed_parts'])}" if _r.get("failed_parts") else ""))
    _run("learn_world", _learn_world)
